...
contract DiceGame is Mortal{
  ...
  function bet(uint _number) payable public {
    /* 베팅넘버가 1 ~ 6 사이고 베팅액이 최소 베팅액 이상일 때 */
    require(_number > 0 && _number <= 6);
    require(msg.value >= minBet);

    uint winningNumber = block.timestamp % 6 + 1;

    if (_number == winningNumber) {
      /* 상금 계산식 */
      uint amountWon = msg.value * (100 - houseEdge)/6;
      /* Win! => 베팅한 사람에게 상금을 송금 */
  ...
  /* 컨트랙트 계좌 잔액 확인 함수 */
  function checkContractBalance(address _myContract) onlyOwner public view returns(uint) {
      address _contract = _myContract;
      return _contract.balance;
  }

  /* fallback 함수 */
  function() payable external {
    revert('something bad happend!');
  }
}
